...
app.post('/api/admin/approve-payment', async (req, res) => {
    try {
        ...
        if (action === 'approve') {
            let referralCode;
            do { referralCode = generateReferralCode(); } while (!isCodeUnique(referralCode, db));
                ...
                if (referee && referee.status === 'approved') {
                    ...
                    if (referee.activeCustomers > 0 && referee.activeCustomers % 5 === 0) {
                        ...
                        const existingPayout = db.payouts.find(p =>
                            p.refereePhone === referee.phone && p.milestone === milestone && p.status === 'pending'
                        );
                        if (!existingPayout) {
                            const newPayout = {
                                id: shortid.generate(),
...
app.get('/api/user/:phone/referrals', (req, res) => {
    try {
        ...
        const referrals = db.users.filter(u => u.refereeCode === referee.referralCode && u.phone !== phone);
...
app.get('/api/user/:phone/payouts', (req, res) => {
    try {
        ...
        const totalPaid = userPayouts.filter(p => p.status === 'completed').reduce((sum, p) => sum + p.amount, 0);

        const nextMilestone = Math.floor((user.activeCustomers || 0) / 5);
        const paidMilestones = userPayouts.filter(p => p.status === 'completed').map(p => p.milestone);
